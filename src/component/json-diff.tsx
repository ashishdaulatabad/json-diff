...
export default function JsonDifference(): JSX.Element {
  ...
  let [summary, setSummary] = React.useState<IterableSummary>(
    {} as IterableSummary,
  );
  ...
  React.useEffect(() => {
    if (isComparing) {
      if (first?.trim().length && second?.trim().length) {
        try {
          compare
            .compare(JSON.parse(first.trim()), JSON.parse(second.trim()))
    ...
    <>
      ...
      <div className="summary p-10 flex justify-center items-center">
        <div className="flex flex-col items-start text-left w-4/5 p-8 border-t-2 border-l-2 border-gray-300 shadow-lg dark:shadow-black dark:border-gray-700 border-r-8 border-b-8 max-h-screen">
          ...
          <div className="w-full mt-4 overflow-y-scroll scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-600 scrollbar-track-gray-300 dark:scrollbar-track-gray-800 scrollbar-thin">
            <DiffSummary
              {...summary}

...
test("1. Test compare empty object", async () => {
...
test("2. Test compare empty arrays", async () => {
  const comparison: Result<IterableSummary, CError> = await compare.compare(
    [],
    [],
  );
...
test("3. Test compare same types", async () => {
  let comparison: Result<IterableSummary, CError> = await compare.compare(
    1222,
    1222,
  );
  expect(comparison.ok()).toBeTruthy();
  ...
  expect(validCompare.summary![0].leftType).toEqual(
    validCompare.summary![0].rightType,
  );
  ...
  expect(validCompare.summary![0].left).not.toEqual(
    validCompare.summary![0].right,
  );

  comparison = await compare.compare(
  ...
  expect(validCompare.summary).toHaveLength(1);
  expect(validCompare.summary![0].diffResult).toEqual(DiffType.Different);
  expect(validCompare.summary![0].leftType).not.toEqual(
    validCompare.summary![0].rightType,
  );
  expect(validCompare.summary![0].left).not.toEqual(
    validCompare.summary![0].right,
...
test("4. Test compare object with content", async () => {
  const comparison: Result<IterableSummary, CError> = await compare.compare(
    { a: "b", e: "g" },
    { c: "d", e: "f" },
  );
  ...
  expect(validCompare.summary!.map((a: Field) => a.fieldKey)).toEqual(
    expect.arrayContaining(["a", "e", "c"]),
  );

  expect(validCompare.summary!.map((a: Field) => a.diffResult)).toEqual(

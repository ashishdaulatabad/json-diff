...
export default {
  ...
  compare: async function (
    ...
    return new Promise((resolve) => {
      const [firstType, secondType] = [Types.ftype(first), Types.ftype(second)];

      /// Normal comparison
      if (firstType === secondType) {
        if (
          Types.typeIsIterable(firstType) &&
          Types.typeIsIterable(secondType)
        ) {
          resolve(
            compareSameIterableType(first, second, [], new Set(), new Set()),
          );
        } else {
...
const compareOneSidedNonIterableTypes = (
  ...
  let remSummary = constructOneSidedIterableType(
    oneSide,
    [],
    iterableSide,
    new Set(),
...
const compareTwoArrays = (
  ...
  for (let index = 0; index < minLength; ++index) {
    ...
    if (firstType === secondType) {
      if (!Types.typeIsIterable(firstType)) {
        const diffResult =
          firstElement === secondElement ? DiffType.Same : DiffType.Different;
      ...
      if (Types.isIterable(firstElement) && Types.isIterable(secondElement)) {
        ...
        Types.isIterable(firstElement) ||
        Types.isIterable(secondElement)
      ) {
  ...
  const [array, side, parentHierarchy] =
    ...
    if (Types.isIterable(element)) {
      ...
      if (inner.ok()) {
        ...
        diffSummary.push({
          diffResult,
          fieldKey: index,
          ...(diffResult === DiffType.LeftOnly && {
            left: element,
            leftType: Types.ftype(element),
          }),
          ...(diffResult === DiffType.RightOnly && {
            right: element,
            rightType: Types.ftype(element),
          }),
          path: path.concat([index]),
          children,
        });
...
const compareTwoObjects = (
  ...
  for (const key of keys) {
    ...
    if (first.hasOwnProperty(key) && second.hasOwnProperty(key)) {
      const firstElement = first[key],
        secondElement = second[key];
      if (Types.eqType(firstElement, secondElement)) {
        ...
        // Types are different
        if (
          Types.typeIsIterable(firstElement) &&
          Types.typeIsIterable(secondElement)
        ) {
          if (leftParentHierarchy.has(firstElement)) {
            return Error({ error: "Cycle found with the first object" });
          }
          ...
          const [element, side, parentHierarchy] = Types.isIterable(
            firstElement,
          )
            ? [firstElement, "left", leftParentHierarchy]
            : [secondElement, "right", rightParentHierarchy];
      ...
      if (Types.isIterable(element)) {
        ...
        if (inner.ok()) {
          ...
          diffSummary.push({
            diffResult:
              side === "left" ? DiffType.LeftOnly : DiffType.RightOnly,
            fieldKey: key,
            path: path.concat([key]),
            ...(side === "left" && {
              left: element,
    ...
    if (Types.isIterable(array[index])) {
      ...
      if (inner.ok()) {
        diffSummary.push({
          ...
          ...(side === "left" && {
            left: array[index],
            leftType: Types.ftype(array[index]),
          }),
          ...(side === "right" && {
            right: array[index],
            rightType: Types.ftype(array[index]),
          }),
          path: path.concat([index]),
          fieldKey: index,
...
const constructForOneSidedObjectOnly = (
  ...
  const incr = side === "left" ? () => (leftonly += 1) : () => (rightonly += 1);
...
const compareDifferentIterableTypes = (
  ...
  const overallSummary = firstSummary.summary?.concat(
    secondSummary.summary as Array<Field>,
  );
...
const constructOneSidedIterableType = (
  ...
  switch (Types.ftype(element)) {
    case Type.Object:
      return constructForOneSidedObjectOnly(
        element,
        path,
        side,
        parentHierarchy,
    ...
    case Type.Array:
      return constructForOneSidedArraysOnly(
        element,
        path,
        side,
        parentHierarchy,
    ...
    default:
      return Error({
        error:
          "Error in detecting proper type of object. Found " +
          Types.type(element),
      });
  }
};
...
const compareSameIterableType = (
  ...
  switch (Types.ftype(first)) {
    ...
    default:
      return Error({
        error: "Unknown type for iterable type. Found " + Types.type(first),
      });
  }
};
